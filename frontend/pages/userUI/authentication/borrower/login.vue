<template>
  <div class="w-full">
    ...
    <div class="mx-auto max-w-xl mt-0 p-6 rounded-2xl">
      <div>
        <h2 class="mb-2 text-center text-3xl font-bold tracking-tight text-gray-900">Welcome back Borrower</h2>
      </div>
    </div>
    ...
    <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-sm">
      <form @submit.prevent="handleLogin" class="space-y-6">
        <!-- Email -->
        <div>
          <label for="email" :class="emailValid ? 'text-blue-500 mb-2 block text-sm font-medium' : 'text-red-600 mb-2 block text-sm font-medium'">Email</label>
        ...
        <!-- Password -->
        <div>
          <label for="password" :class="passwordValid ? 'text-blue-500 mb-2 block text-sm font-medium' : 'text-red-600 mb-2 block text-sm font-medium'">Password</label>
      ...
      <div class="flex justify-between mx-2 my-3">
        ...
        <NuxtLink to="/userUI/authentication/borrower/signUp" class="text-blue-500 text-sans">Sign up</NuxtLink>
      </div>
    </div>
  </div>
</template>
...
const  handleLogin= async ()=> {
  ...
  if (!valid)  return
    ...
    const res = await $fetch<loginResponse>(`${config.public.sanctum.baseUrl}/borrower/login`, {
      method:'POST',
      body: {
        email:form.email,
