<template>
  <div class="w-full">
    ...
    <div class="mx-auto max-w-xl mt-0 p-6 rounded-2xl">
      <div>
        <h2 class="mb-2 text-center text-3xl font-bold tracking-tight text-gray-900">Welcome back Lender</h2>
      </div>
    </div>
    ...
    <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-sm">
      ...
      <div class="flex justify-between mx-2 my-3">
        ...
        <NuxtLink to="/useruI/authentication/lender/signUp" class="text-blue-500 text-sans">Sign up</NuxtLink>
      </div>
    </div>
  </div>
</template>
...
const loginSubmit=async() =>{
  ...
  try{
    const res=await $fetch<loginResponse>(`${config.public.sanctum.baseUrl}/lender/login`, {
      method:'POST',
      body: {
        email:form.email,
