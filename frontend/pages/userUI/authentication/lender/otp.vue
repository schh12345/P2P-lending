<template>
  <div>
    <div v-if="flashMessage" class="text-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
...
const verifyOTP = async() => {
  ...
  try{
    ...
    const res=await $fetch<otpResponse>(`${config.public.sanctum.baseUrl}/lender/verifyOTP`, {
      method:'POST', 
      headers:{
    ...
    alert(err?.data?.message || 'opt verification fail')
  }
}
