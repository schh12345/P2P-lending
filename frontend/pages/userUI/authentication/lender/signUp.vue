<template>
  <div class="w-full min-h-220">
    ...
    <div class="mx-auto max-w-xl mt-0 p-6 rounded-2xl">
      <div>
        <h2 class="mb-2 text-center text-3xl font-bold tracking-tight text-gray-900">Welcome Lender</h2>
        <p class="text-gray-600 text-center text-md">Join our trust platform, start investing today</p>
      </div>
      ...
      <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form @submit.prevent="handlesubmit" class="space-y-6">
          ...
          <div>
            ...
            <input type="string" id="phone_number" v-model="form.phone_number"
              class="block w-full shadow-sm rounded-md bg-white px-3 py-1.5 text-base text-gray-900 border border-gray-300 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-600" />
        ...
        <div class="flex justify-between mx-2 my-3">
          ...
          <NuxtLink to="/userUI/authentication/lender/login" class="text-blue-500 text-sans">Login</NuxtLink>
        </div>
      </div>
    </div>
  </div>
...
const handlesubmit=async() =>{
  ...
  if (!valid)  return
    ...
    try{
      ...
      const res= await $fetch<RegisterResponse>(`${config.public.sanctum.baseUrl}/lender/register`, {
        method:'POST',
        body: {
      ...
      if(res && res.token) {
        ...
        router.push('/userUI/authentication/lender/otp')
      } else {
