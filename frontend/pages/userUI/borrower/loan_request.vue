        ...
        <!-- Loan Details Form -->
          ...
          <div class="bg-white rounded-2xl p-6 shadow space-y-4">
              ...
              <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                  ...
                  <input type="number" v-model="form.amount"  min="100" max="10000" class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" placeholder="0.00" />
              </div>
              ...
              <!--lona interest rate-->
              <div>
              <label class="block text-sm font-medium">Proposed Interest Rate*</label>
              <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                  <div class="shrink-0 text-base text-gray-500 select-none sm:text-sm/6">%</div>
              ...
              <div>
                ...
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                    ...
                    <input type="number" v-model="form.duration" class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" placeholder="0.00" />
                    <div class="shrink-0 text-base text-gray-500 select-none sm:text-sm/6 px-3">|</div>
                    <div class="shrink-0 text-base text-gray-500 select-none sm:text-sm/6 pr-3">days</div>
                </div>
              ...
              <div>
                ...
                <input type="text" v-model="form.reason" placeholder="Home Improvement"
...
const total = computed(() => {
  ...
  return (
    Number(form.amount) * (1 + ((interest_rate.value / 100) * (Number(form.duration) / 30)))
  ).toFixed(2)
})
...
const handleRequest = async () => {
 ...
 console.log(id)
  ...
  try {
  await $fetch(`${config.public.sanctum.baseUrl}/loanRequest`, {
    method: 'POST',
    body: {
      request_amount: form.amount,
      request_duration: form.duration,
...
// const fullUrl = (path: string) => `http://localhost:8000${path}`
// const loans = ref<Loan[]>([])
// const loading = ref(true)
// onMounted(async()=> {
//   try {
//     const loanRes = await $fetch<{loans: Loan[]}>(`${config.public.sanctum.baseUrl}/allLoan/${borrowerId.value}`, {
//       headers: {
//         Authorization: `Bearer ${token.value}`,
//       },
//     })
