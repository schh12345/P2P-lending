    ...
    <div class="p-4 md:p-8">
        ...
        <!-- Loan Details Form -->
          ...
          <div class="bg-white rounded-2xl p-6 shadow space-y-4">
              ...
              <label class="block text-sm font-medium">Loan Amount*</label>
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                    ...
                    <div class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6">
              ...
              <div>
                ...
                <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                    ...
                    <div class="shrink-0 text-base text-gray-500 select-none sm:text-sm/6 px-3">|</div>
                    <div class="shrink-0 text-base text-gray-500 select-none sm:text-sm/6 pr-3">months</div>
                </div>
              ...
              <div>
                <label class="block text-sm font-medium">Reason for Loan</label>
                  <div class="mt-1 w-full rounded-md border border-gray-300 px-4 py-2">
          ...
          <div class="mt-6 bg-white p-6 shadow rounded-2xl space-y-4">
              <h3 class="font-bold text-lg text-gray-800 mb-2 text-center">Preview Loan Requesting</h3>
              <div class="space-y-3 text-lg border-b-1 border-gray-200 text-gray-900">
          ...
          <div class="mt-6 bg-white p-6 shadow rounded-2xl space-y-4">
              ...
              <div class="space-y-5 text-sm">
                  ...
                  <div class="space-y-1">
                      ...
                      <select v-model="form.emp_status" class="w-full rounded-md bg-white p-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                  ...
                  <div class="space-y-1">
                      ...
                      <div class="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                          <div class="shrink-0 text-base text-gray-500 select-none sm:text-sm/6">$</div>
                          <input type="number" v-model="form.income"  min="100" max="5000" class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" placeholder="0.00" />
                      </div>
...
const errors = reactive({
  amount: '', duration: '', reason: '', emp_status: '', income: '', imageFile: '', imageFileEmp: ''
})
...
onMounted(async () => {
  try {
    ...
    const res = await $fetch(`${config.public.sanctum.baseUrl}/loan/${loanId}`, {
...
const total = computed(() => {
  ...
  if (!loan.request_amount || !loan.interest_rate || !loan.request_duration) return 0

  return (
    loans.value.request_amount * (1 + ((loans.value.interest_rate / 100) * (loans.value.request_duration / 30)))
  ).toFixed(2)
})
...
const handleRequest = async () => {
  ...
  let valid = true
  if (!form.amount) { errors.amount = 'Amount is required'; valid = false }
  else if (form.amount < 100 || form.amount > 5000) {
    errors.amount = 'Amount must be between 100 to 5000'; valid = false
  }

  if (!form.duration) { errors.duration = 'Duration is required'; valid = false }
  else if (form.duration > 6) {
    errors.duration = 'Duration must be 6 months or less'; valid = false
  }

  if (!form.reason) { errors.reason = 'Reason is required'; valid = false }
  if (!form.emp_status) { errors.emp_status = 'Employment status is required'; valid = false }
  if (!form.income) { errors.income = 'Income is required'; valid = false }
  ...
  try {
    ...
    if (imageFile.value || imageFileEmp.value) {
      ...
      const uploadRes = await $fetch(`${config.public.sanctum.baseUrl}/storeImage`, {
        method: 'POST',
        body: formData,
        headers: { Authorization: `Bearer ${token.value}` },
      })

      if (uploadRes.identity_path) identity_path.value = uploadRes.identity_path
      if (uploadRes.employment_path) employment_path.value = uploadRes.employment_path
    }

    // Final update
    const updateRes = await $fetch(`${config.public.sanctum.baseUrl}/loanUpdate/${route.params.id}`, {
        method: 'POST',
        body: {
