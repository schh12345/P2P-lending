<template>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
            ...
            <div class="md:flex md:flex-row md:justify-between flex flex-col">
                <!--total loan-->
                <div class="p-10 bg-white rounded-lg border-1 border-gray-100 flex justify-start items-center gap-x-5">
                ...
                <!--Completed loan-->
                <div class="p-10 bg-white rounded-lg border-1 border-gray-100 flex justify-start items-center gap-x-5">
                    <img :src="completedLoan" alt="completed loan image" class="w-auto h-auto">
                    <div>
                        <p class="text-gray-600 text-md">Completed Funds</p>
                        <!--fetch from table laon of user-->
                        <p class="text-4xl font-bold text-gray-900">{{ completedLoanCount }}</p>
                    </div>
                </div>
            ...
            <div class="rounded-lg overflow-hidden space-y-5">
                ...
                <!-- Tab Navigation -->
                 <div v-else>
                <div class="bg-white flex flex-wrap border-b-2 border-gray-200 text-sm sm:text-base">
                    <button
                    v-for="tab in tabs"
                    :key="tab.id"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 border-separate border-spacing-y-5">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <td class="px-4 py-3 sm:px-6 whitespace-nowrap">
                            ...
                            >
                                {{ item.lender?.first_name.charAt(0).toUpperCase() }}{{ item.lender?.last_name.charAt(0).toUpperCase() }}
                            </div>
                        ...
                        <td class="px-4 py-3 sm:px-6 whitespace-nowrap text-sm text-gray-100">
                            <NuxtLink :to="`/userUI/borrower/payback/${item.id}`" v-if="item.status!== 'completed'" class="bg-blue-500 py-2 px-5 rounded-xl">
                                Pay Now
                            </NuxtLink>
                            <NuxtLink to="#" v-if="item.status==='completed'" class="bg-green-500 py-2 px-5 rounded-xl">
                                Review
                            </NuxtLink>
                        </td> 
...
<script setup>
    ...
    onMounted(async()=> {
        ...
        try{
            ...
            const res = await $fetch(`${config.public.sanctum.baseUrl}/getAllLoanAfterApproveforBorrower/${id}`, {
                headers: {
    ...
    const activeLoanCount = computed(() => {
        return loans.value?.filter(loan => loan.status?.toLowerCase() === 'active').length || 0
    })

    // Completed loans count
    const completedLoanCount = computed(() => {
        return loans.value?.filter(loan => loan.status?.toLowerCase() === 'completed').length || 0
    })

    // Late loans count
    const lateLoanCount = computed(() => {
        return loans.value?.filter(loan => loan.status?.toLowerCase() === 'late').length || 0
    })
</script>
