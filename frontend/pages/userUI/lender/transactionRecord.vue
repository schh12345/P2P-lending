            ...
            <!-- Transaction History -->
             ...
             <div v-else>
              <div v-if="transactions" class="p-4 sm:p-6 w-full  mt-6 md:mt-0">
                  <p class="text-lg font-semibold text-gray-700 mb-4">Transactions Record</p>
                  ...
                  >
                      ...
                      <div class="flex items-start gap-4 w-full sm:w-auto flex-1">
                          ...
                          >
                          <span class="w-5 h-5 block" v-html="transaction.type === 'repayment' ? getIcon('arrowDownLeft') : getIcon('arrowUpRight')"></span>
                          </div>
                      ...
                      <div class="text-right w-full sm:w-auto">
                          ...
                          :class="[
                              ...
                              transaction.type === 'repayment' ? 'text-green-600' : 'text-red-600'
                          ]"
                          >
                          {{ transaction.type === 'repayment' ? '+' : '-' }}${{ formatAmount(transaction.amount) }}
                          </p>
...
<script setup>
  ...
  import { useAuthLender } from '~/composables/useAuthLender';
    ...
    onMounted(async()=> {
      ...
      try {
        const res = await $fetch(`${config.public.sanctum.baseUrl}/transactionForLender/${id}`, {
          headers: {
