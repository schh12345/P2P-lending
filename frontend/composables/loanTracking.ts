...
export const loanTracking=async()=> {
    ...
    try{
        ...
        const res = await $fetch<LoanTrackingResponse>(`${config.public.sanctum.baseUrl}/loanTracking/${id}`, {
            headers: {
        ...
        if (res && res.alert.length) {
                ...
                const formattedAlerts: string[] = laonTrackingRes.value.map(
                    ([firstName, lastName, lateTime]) => `you are late ${lateTime} for the fund from ${firstName} ${lastName}`
                );
